...
/** 세션 생성 응답 — 합의 계약(id 포함) 대비 임시 확장 타입.
    현행 배포 스키마(SessionTokenResponse)에는 id가 없으므로 optional로 취급하고,
    백엔드 반영 후 schema.ts 재생성 시 이 확장을 제거한다 */
export type CreateSessionResponse = components["schemas"]["SessionTokenResponse"] & {
  id?: number;
};
