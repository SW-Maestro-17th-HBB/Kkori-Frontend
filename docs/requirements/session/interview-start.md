...
/setup: ① 이력서(분석 완료만·30분 필수) ② 직무 position(추천 기본값) ③ 시간(5/30분)
...
- **① 이력서 선택**: 분석 완료(`status === "done"`) 이력서만 드롭다운에 노출한다. 선택 상태는 이력서 **id(number)** 로 관리한다(현재 name 문자열 상태를 교체 — API 계약이 id 기반). 선택 시 파일명·"분석 완료"를 표시하는 기존 UI를 유지한다.
  ...
  - **선택 해제**: 드롭다운에 "이력서 선택 안 함" 옵션을 제공한다 — 프리셀렉트되었거나 이미 선택한 뒤에도 이력서 없는 빠른 연습(5분)으로 되돌릴 수 있어야 한다(이력서 선택적 요구의 완결).
- **② 직무(position)**: `BACKEND`("백엔드") · `FRONTEND`("프론트엔드") 두 값의 커스텀 드롭다운. 기존 정적 표시(SelectRow)를 실제 선택 UI로 교체한다.
  - **추천 기본값**: 사용자가 직접 선택하기 전에는 선택된 이력서의 추천 직무(`Resume.recommendedPosition` — 분석 결과에서 파생, 픽스처 추가)를 따르고, 이력서를 바꾸면 추천도 따라간다. 사용자가 직접 선택한 뒤에는 이력서를 바꿔도 덮어쓰지 않는다(명시 선택 우선). 파생 규칙: `effective = userPosition ?? selectedResume.recommendedPosition ?? "BACKEND"`.
  - 안내 문구 — `recommendedPosition`이 optional이므로 세 갈래로 분기한다:
    - 이력서 선택 + 추천값 있음: "이력서를 분석해 **{추천 직무}** 로 추천했어요. 직무를 바꾸면 질문 방향이 달라져요."(기존 문구 유지)
    - 이력서 선택 + 추천값 없음: "기본 직무는 백엔드예요. 직무를 바꾸면 질문 방향이 달라져요."
    - 이력서 미선택: "이력서를 선택하면 직무를 추천해 드려요."
- **③ 면접 시간**: 빠른 연습(5분) / 실전 모의(30분) 카드, 기본 선택은 빠른 연습(기존 유지).
  - **실전 모의(30분)는 이력서 필수** — 이력서 미선택 시 잠금(`.time-opt--locked`)과 안내 문구를 유지한다.
  - **빠른 연습(5분)은 이력서 선택적** — 이력서 없이 시작 가능하고, 선택했다면 세션 생성에 반영한다(질문 개인화). 시간 선택이 곧 면접 유형으로 전송된다 — `interviewType: "FIVE_MIN"`(5분) / `"THIRTY_MIN"`(30분).
  - 이력서 없이 실전 모의가 선택된 조합은 렌더 시 파생 값으로 빠른 연습으로 강등하는 기존 로직(`effectiveDur`)을 유지한다.
- **시작 게이팅**: "면접 시작" 활성 조건 = ④장비 점검 통과(`canStart`, HBB1-19) **AND** (빠른 연습 **OR** 이력서 선택됨) **AND** 세션 생성 요청 중 아님. 비활성 사유 안내는 장비 점검 문구(HBB1-19)를 유지한다.
...
- ① 프리셀렉트·선택 후 "이력서 선택 안 함"으로 해제하면 미선택 상태로 돌아가는지 확인
- ② 직무 기본값이 선택된 이력서의 추천 직무를 따르는지 확인 — 이력서 변경 시 추천도 변경
- ② 직무를 직접 선택한 뒤에는 이력서를 바꿔도 선택이 유지되는지 확인
- ② 이력서 미선택 시 추천 문구 대신 선택 유도 문구가 표시되는지 확인
- ② 추천 직무가 없는 이력서를 선택하면 기본 직무 안내 문구가 표시되는지 확인(허위 추천 문구 없음)
- ③ 이력서 미선택 시 실전 모의가 잠기고 빠른 연습만 선택 가능한지 확인(기존 동작 회귀 없음)
- 이력서 미선택 + 빠른 연습 + 장비 점검 통과 조합에서 "면접 시작"이 활성화되는지 확인
...
- 타입: `src/api/types.ts` — `Position = "BACKEND" | "FRONTEND"`, `Resume.recommendedPosition?: Position`(분석 완료 시)
- 픽스처: `src/api/fixtures.ts` — 분석 완료 이력서에 `recommendedPosition` 부여
...
```
  ...
  - `resumeId`: 실전 모의(30분)는 필수(FE 게이팅이 보장), 빠른 연습(5분)은 이력서를 선택한 경우에만 포함(미선택 시 필드 생략).
...
- 빠른 연습 + 이력서 미선택 요청 본문에 `interviewType: "FIVE_MIN"`이 있고 `resumeId` 필드가 없는지 확인
- 빠른 연습 + 이력서 선택 요청 본문에 `resumeId`가 포함되는지 확인
- ② 선택값이 `position`으로 전송되는지 확인 — 이력서 미선택 상태에서도 기본값 `"BACKEND"`가 전송됨
