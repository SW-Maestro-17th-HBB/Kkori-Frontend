...
- **저장**: 발급 후 **LiveKit 접속까지 성공하면**(기준·순서는 아래 "접속 확립 후 저장·이동") sessionStorage `hbb.interview.session`에 `{ url, token, room, authSessionId, id }`를 저장하고 `/live`로 이동한다. `url`·`token`은 LiveKit 접속용, `room`은 응답의 룸 이름(표시·디버깅용), `authSessionId`는 **요청 시작 직전에 캡처한 인증 세션 ID**(`getAuthSnapshot().sessionId`) — 계정 교체 감지용(캡처·재대조 규칙은 아래). 세션 `id`는 **필수 저장**한다(종료·재연결 등 후속 세션 API의 key). 새로 발급받으면 덮어쓴다.
